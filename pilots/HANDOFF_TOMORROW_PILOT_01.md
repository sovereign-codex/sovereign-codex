...
Can a fresh steward reconstruct enough of the institution to continue one bounded responsibility without access to private conversational continuity?
...
The steward must independently report:

1. Origin — why the work exists.
2. Invariants — what may not be silently violated.
3. Lineage — how the current architecture became what it is.
...
- `RECONSTRUCTED` — supported from governed artifacts with no hidden-memory assistance.
- `PARTIAL` — directionally correct but important ambiguity remains.
- `CONTRADICTORY` — governed artifacts support incompatible interpretations.
- `MISSING` — required inheritance is absent.
- `OVERREACHED` — steward inferred authority or certainty not supported by artifacts.

## Inheritance debt record

Every failure becomes a debt item:

```yaml
debt_id:
class: lineage|status|authority|evidence|custody|semantic|priority
observed_failure:
missing_or_conflicting_artifacts: []
consequence_if_unrepaired:
proposed_repair:
repair_owner:
...
Pilot 01 passes only when the fresh steward can:

- reconstruct the institutional purpose and current root priority;
...
1. a reconstruction report;
2. an inheritance-debt ledger;
3. proposed artifact repairs;
4. a revised succession envelope if warranted; and
5. evidence for Canon Steward review of the charter.
